...
아래와 같은 결과가 출력될 것입니다. 이 결과 중에 세번째 컬럼은 EC2 인스턴스의 고유의 ID를 의미합니다. 모든 랩을 진행하는 동안 계속 사용되므로 저장해 둡니다.
...
아래와 같이 SSM\(System Session Manager\)를 통해 Private IP로 할당된 각 VPC EC2 인스턴스에 접속합니다.
