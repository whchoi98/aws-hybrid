...
**11. EU-CENTRAL-1 리전에서 Transit Gateway Route Table - Frankfurt Route Table에 Routing Table에서 F OCTANK VPC 라우팅 전파를 확인합니다.**
...
**12. EU-CENTRAL-1 리전에서 Transit Gateway Route Table - Frankfurt Route Table에 US-EAST-1의 VPC들의 CIDR 주소로 라우팅 될 수 있도록 Static 라우팅 테이블을 추가합니다.** 

{% hint style="info" %}
10.0.0.0/8  로 라우팅을 Supernetting으로 처리하는 이유는 수신측 라우팅 테이블에서 별도로 10.101.0.0/16 경로에 대해 알지 못하면 네트워크 트래픽에 대한 응답이 없기 때문입니다.

만약 Suppernetting을 하지 
{% endhint %}
